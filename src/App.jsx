...
function App() {
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-slate-200 flex flex-col items-center justify-center p-6 font-sans selection:bg-indigo-500/30">
      {/* Background Decor */}
      ...
      <div className="w-full max-w-[400px] relative">
        {/* Glass Card */}
        <div className="bg-slate-900/40 backdrop-blur-2xl p-6 rounded-[32px] border border-white/10 shadow-2xl overflow-hidden">
          
          {/* Header */}
          <div className="flex items-center justify-between mb-8">
            <div className="flex items-center gap-3">
              <div className="p-2 bg-indigo-500/20 rounded-xl">
                <CalcIcon className="text-indigo-400" size={18} />
              </div>
              <span className="text-xs font-bold uppercase tracking-[0.2em] text-slate-400">Quantum Calc</span>
            </div>
            <button 
              onClick={() => setIsScientific(!isScientific)}
              className="p-2 hover:bg-white/5 rounded-xl transition-colors text-slate-400 hover:text-indigo-400"
            >
          ...
          <div className="relative mb-8 px-2">
            <div className="h-6 text-right text-indigo-400/60 text-sm font-medium mb-1 transition-all">
              {prevValue !== null ? `${prevValue} ${operator || ''}` : history[0]?.split('=')[0]}
            </div>
            <div className="text-right overflow-hidden">
              <span className="text-6xl font-light tracking-tighter text-white drop-shadow-[0_0_15px_rgba(255,255,255,0.1)]">
                {display}
              </span>
            </div>
          </div>
          ...
          {isScientific && (
            <div className="grid grid-cols-4 gap-2 mb-4 animate-in fade-in slide-in-from-top-4 duration-300">
              <Button onClick={() => advancedFunc('sin')} variant="advanced" size="sm">sin</Button>
              <Button onClick={() => advancedFunc('cos')} variant="advanced" size="sm">cos</Button>
              <Button onClick={() => advancedFunc('log')} variant="advanced" size="sm">log</Button>
              <Button onClick={() => performOperation('^')} variant="advanced" size="sm">xʸ</Button>
              <Button onClick={() => advancedFunc('sqrt')} variant="advanced" size="sm">√x</Button>
              <Button onClick={() => advancedFunc('pow2')} variant="advanced" size="sm">x²</Button>
              <Button onClick={() => advancedFunc('abs')} variant="advanced" size="sm">|x|</Button>
              <Button onClick={() => setDisplay(String(Math.PI))} variant="advanced" size="sm">π</Button>
            </div>
          )}
          ...
          <div className="grid grid-cols-4 gap-3">
            {/* Row 1 */}
            <Button onClick={clear} variant="danger" className="col-span-2">
              <RotateCcw size={18} className="mr-2 opacity-70" /> CLEAR
            </Button>
            ...
            {/* Row 5 */}
            <Button onClick={() => inputDigit(0)} variant="number" className="col-span-2 text-left px-8">0</Button>
        ...
        {/* History Widget */}
        <div className="mt-6 bg-white/5 backdrop-blur-lg rounded-2xl border border-white/5 p-4">
          ...
          <div className="space-y-2">
            {history.length > 0 ? history.map((item, i) => (
              <div key={i} className="text-xs text-slate-500 flex justify-between border-b border-white/5 pb-1">
                <span>{item.split('=')[0]}</span>
                <span className="text-indigo-400 font-medium">= {item.split('=')[1]}</span>
              </div>
            )) : (
              <div className="text-[10px] text-slate-600 italic">No calculations yet...</div>
            )}
          </div>
        </div>
      </div>

      <div className="mt-12 flex items-center gap-6 text-slate-600 text-[10px] font-medium tracking-[0.2em] uppercase">
        <div className="flex items-center gap-2">
          <Zap size={12} className="text-amber-500" /> ULTRA RESPONSIVE
        </div>
        <div className="flex items-center gap-2">
          <Activity size={12} className="text-indigo-500" /> QUANTUM ENGINE
        </div>
      </div>
    </div>
  )
